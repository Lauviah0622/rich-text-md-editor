...
export const isTextJsxElement = (element: MdxJsxFlowElement): element is Text => {
  return element.element.name === 'Text'
}

type AttributesRecord = Record<Attributes[number]['name'] | string, Attributes[number]['value']>

export const Render = ({ element }: Pick<CustomRenderElement<Text>, 'element'>) => {
  const editor = useSlate()

  // TODO Use Map instead object
  const attributesRecord = element.element.attributes.reduce<AttributesRecord>((record, { name, value }) => {
    record[name] = value
  ...
  const update = (name: string, value: any) => {
    ...
    // const targetElement = editor.selection && Node.get(editor, [at])

    console.log('attributes', attributes)
    console.log('at', at, editor.selection?.anchor.path)
    // console.log('targetElement', targetElement)
  }

  return <div >
    mdxJsxTextElement:<br/>
    <select name="align" value={attributesRecord.align} onChange={(e) => {
